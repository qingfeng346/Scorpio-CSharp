...
namespace Scorpio
{
    ...
    public abstract class ScriptObject
    {
        ...
        //调用函数
        public virtual object Call(ScriptObject[] parameters) { throw new ExecutionException(m_Script, this, "类型[" + Type + "]不支持函数调用"); }
        //两个数值比较 > >= < <=
        public virtual bool Compare(TokenType type, ScriptObject obj) { throw new ExecutionException(m_Script, this, "类型[" + Type + "]不支持值比较[" + type + "]"); }
        //运算符或者位运算 + - * / % | & ^ >> <<
        public virtual ScriptObject Compute(TokenType type, ScriptObject obj) { throw new ExecutionException(m_Script, this, "类型[" + Type + "]不支持运算符[" + type + "]"); }
        //运算符或者位运算赋值运算 += -= *= /= %= |= &= ^= >>= <<=
        public virtual ScriptObject AssignCompute(TokenType type, ScriptObject obj) { throw new ExecutionException(m_Script, this, "类型[" + Type + "]不支持赋值运算符[" + type + "]"); }
