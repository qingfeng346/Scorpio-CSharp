...
namespace Scorpio.Library
{
    public partial class LibraryBasis {
        ...
        private class MapPairs : ScorpioHandle {
            ...
            public MapPairs(Script script, ScriptMap itorResult, IEnumerator<KeyValuePair<object, ScriptValue>> enumerator) {
                this.script = script;
        ...
        private class toIndex : ScorpioHandle {
            ...
            public ScriptValue Call(ScriptValue thisObject, ScriptValue[] args, int length) {
                ...
                if (value.valueType == ScriptValue.scriptValueType) {
                    return new ScriptValue($"{value.scriptValue}  index:{value.index}  id:{value.scriptValue.Index}  count:{ScriptObjectReference.GetReferenceCount(value.index)}");
