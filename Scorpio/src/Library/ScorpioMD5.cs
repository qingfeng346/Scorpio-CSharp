...
namespace Scorpio.Library {
    public class ScorpioMD5 : IDisposable {
        static public string GetMd5String(string source) {
            return GetMd5String(Encoding.UTF8.GetBytes(source));
        }
        static public string GetMd5String(byte[] buffer) {
            return HashToString(GetMd5Bytes(buffer));
        }
        static public string GetMd5String(byte[] buffer, int offset, int length) {
            return HashToString(GetMd5Bytes(buffer, offset, length));
        }
        static public string GetMd5String(Stream stream) {
            return HashToString(GetMd5Bytes(stream));
        }
        static public byte[] GetMd5Bytes(string source) {
            return GetMd5Bytes(Encoding.UTF8.GetBytes(source));
        }
        static public byte[] GetMd5Bytes(byte[] buffer) {
            using (ScorpioMD5 md = new ScorpioMD5()) {
                return md.ComputeHash(buffer, 0, buffer.Length);
            }
        }
        static public byte[] GetMd5Bytes(byte[] buffer, int offset, int length) {
            using (ScorpioMD5 md = new ScorpioMD5()) {
                return md.ComputeHash(buffer, offset, length);
            }
        }
        static public byte[] GetMd5Bytes(Stream stream) {
            using (ScorpioMD5 md = new ScorpioMD5()) {
