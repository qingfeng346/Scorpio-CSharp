...
namespace Scorpio.Library {
    public class LibraryIO {
        ...
        public static void Load(Script script) {
            ...
            script.AddLibrary("io", functions);
        }
        ...
        private class readAllLines : ScorpioHandle {
            ...
            public ScriptValue Call(ScriptValue thisObject, ScriptValue[] args, int length) {
                var lines = File.ReadAllLines(args[0].ToString(), length > 1 ? Encoding.GetEncoding(args[1].ToString()) : DefaultEncoding);
                var array = new ScriptArray(script);
