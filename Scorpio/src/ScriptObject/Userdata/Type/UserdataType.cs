...
namespace Scorpio.Userdata {
    ...
    public abstract class UserdataType {
        ...
        //获取一个内部类
        protected bool TryGetNestedType(string name, out ScriptValue value) {
            var nestedType = m_Type.GetNestedType(name, Script.BindingFlag);
            if (nestedType == null) {
                value = default;
                return false;
            }
            m_Values[string.Intern(name)] = value = ScorpioTypeManager.GetUserdataType(nestedType);
            return true;
        }
        //获得操作符重载函数
        ...
        public object GetStaticValue(string name) {
            ...
            if (userdataMethod != null) return m_StaticMethods[name] = new ScriptValue(new ScriptStaticMethodFunction(userdataMethod));
            if (TryGetNestedType(name, out value)) return value;
            throw new ExecutionException($"GetValue Type:[{m_Type.FullName}] 静态变量:[{name}]不存在");
        }
        //获取实例变量
        public object GetInstanceValue(string name, object obj) {
            if (m_InstanceMethods.TryGetValue(name, out var value)) return value;
            if (m_Values.TryGetValue(name, out value)) return value;
            if (TryGetValue(obj, name, out var val)) return val;
            var userdataMethod = GetMethod(name);
            if (userdataMethod != null) return m_InstanceMethods[name] = new ScriptValue(new ScriptInstanceMethodFunction(userdataMethod));
